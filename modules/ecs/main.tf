...
======*/
resource "aws_cloudwatch_log_group" "app02-iac" {
  name = "app02-iac"

  tags = {
    Environment = "${var.environment}"
    Application = "app02-iac"
  }
}
...
======*/
resource "aws_ecr_repository" "app02-iac" {
...
data "template_file" "web_task" {
  ...
  vars = {
    image           = "${aws_ecr_repository.app02-iac.repository_url}"
    ...
    log_group       = "${aws_cloudwatch_log_group.app02-iac.name}"
  }
}
...
resource "aws_alb" "alb_app02-iac" {
  name            = "${var.environment}-alb-app02-iac"
...
resource "aws_alb_listener" "app02-iac" {
  load_balancer_arn = "${aws_alb.alb_app02-iac.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_app02-iac_web_cpu_utilization_high"
